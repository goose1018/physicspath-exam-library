"""Extract raw_text.txt + images from 2023 全国乙卷 DOCX.
Output format follows PhysicsPath convention:
  raw_text.txt:  [P{i}] {paragraph text}  (one paragraph per line)
  images/imageN.{png,wmf,...}
"""
import os, shutil
...
SRC = r"C:/Users/60507/Desktop/17-24物理/2023·高考物理真题/2023年高考物理试卷（全国乙卷）（解析卷）.docx"
OUT = Path(r"C:/Users/60507/physics_anim/output/2023_全国乙卷/source")
IMG_OUT = OUT / "images"
...
# Copy all images by reading the part bytes (works across formats)
seen = set()
for rid, rel in rels.items():
...
print(f"  raw_text.txt -> {OUT/'raw_text.txt'}")
