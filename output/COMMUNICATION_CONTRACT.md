...
### 1.2 汇报必须包含
- ✅ 4 个 agent 的真实 ID（`tool_use_id`，不能编造）
- ✅ 每关具体发现（不能只写"通过")
...
### 3.1 4 关 = 4 个独立 sub-agent（绝不合并）

```
A 关 题目识别 → 1 个独立 Agent 调用（独立 prompt）
B 关 物理推导 → 1 个独立 Agent 调用（独立 prompt）
C 关 视觉直觉 → 1 个独立 Agent 调用（独立 prompt）
D 关 代码审   → 1 个独立 Agent 调用（独立 prompt）
共 4 个 Agent tool 调用，可并行（同一 message 多个调用块），但 prompt 必须独立
```
