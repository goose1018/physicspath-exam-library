...
## 13. drawScene 文字规则（2026-04-30 用户强制）

用户原话「不用花 token 写题目讲解 ... 你只需要确保我们花的动态图符合正确答案与物理逻辑就行了 还省时间 ... 但是下面这一栏 KaTeX 讲解必须保留」

### 13.1 绝对禁令

- ❌ NEVER 在 drawScene 内用 ctx.fillText 手写多行**文字推导面板**（"对称性论证 · 4 项判断"、"论证 ① a/b"、"撤 F 前匀速：F=2μmg" 这类清单式讲解）
- ❌ NEVER 新建 drawAnalysis / drawForceAnalysis / drawDerivation 这种"画布内全是文字"的子面板

### 13.2 必须保留

- ✅ **视觉/动画**（滑块、弹簧、轨迹、力箭头、几何小图、阶段标签）— 这才是 drawScene 的本职
- ✅ **答案徽章**（u≥0.92 显示的"答案 X"框）— 醒目但只一个
- ✅ **关键物理量短标注**（紧贴几何的单行文字，如 "v_P=0.40"、"弹簧伸长≈0.6"、"y_max=2R"）
- ✅ phases[].formula_tex —— 这是 KaTeX 公式栏（画布下方），完整推导链条放这里
- ✅ stats / conclusions / legend / officialAnswer —— 这些是文字讲解的归宿

### 13.3 为什么

- 文字讲解已经在 KaTeX 公式栏 + officialAnswer 覆盖；画布再写是冗余
- 画布的核心是"动态图符合正确答案与物理逻辑"
- 节省 token 让会话能完成更多题目

### 13.4 5 关 sub-agent 审核同步

C2 关（视觉冗余）发现 drawScene 内有大段 fillText 文字推导清单时 → 直接标 ❌ FAIL，要求删除。

### 13.5 适用范围

2026-04-30 起所有新题目；旧题目不回炉，接手维护时按此规则简化。
